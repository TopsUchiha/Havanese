...
router.post(
  ...
  (req, res) => {
    ...
    const { name, age, category, description, price } = req.body;
...
router.put(
  ...
  (req, res) => {
    ...
    db.get('SELECT image_url FROM puppies WHERE id = ?', [req.params.id], (fetchErr, existingPuppy) => {
      ...
      db.run(
        `UPDATE puppies SET name = ?, age = ?, category = ?, description = ?, image_url = ?, price = ? WHERE id = ?`,
        [name, age, category, description, image_url, price, req.params.id],
        function (updateErr) {
          if (updateErr) {
            if (req.file) fs.unlink(req.file.path, () => {});
            return res.status(500).json({ error: 'Failed to update puppy listing.' });
