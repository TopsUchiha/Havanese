// config/db.js
// SQLite database configuration, schema auto-creation, and seed data logic.
...
// -----------------------------------------------------------------------------
// Seed puppies: 6 Adoption + 6 Rescue, with high-quality Unsplash image URLs.
// Each frontend <img> tag has an onerror fallback, so even if an individual
// Unsplash URL ever goes stale, the layout never breaks or shows a broken icon.
// -----------------------------------------------------------------------------
const SEED_PUPPIES = [
  // ---------------------------- ADOPTION (6) ----------------------------
  {
...
function createTables(callback) {
  db.serialize(() => {
    ...
    // New table: donation requests (name, email, amount, chosen payment method)
    db.run(
  ...
  db.get('SELECT COUNT(*) AS count FROM puppies', (err, row) => {
    ...
    if (row.count === 0) {
      ...
      SEED_PUPPIES.forEach((p) => {
        stmt.run([p.name, p.age, p.category, p.description, p.image_url]);
      });

      stmt.finalize((finalizeErr) => {
        if (finalizeErr) return callback(finalizeErr);
        console.log('Seeded 12 sample Havanese puppies (6 Adoption, 6 Rescue).');
        callback(null);
      });
    } else {
      callback(null);
